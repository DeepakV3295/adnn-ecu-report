...
The GNU time program is used to measure benchmark application's execution time and peak memory usage. After each training epoch of the application, the model accuracy is evaluated on the MNIST test set. The accuracy measured after 30 epochs is then used for the analysis. This experiment is repeated 10 times, and the final values are the average of all iterations.
