...
\begin{center}
	\begin{tikzpicture}[x=2.4cm,y=1cm]
		\readlist\Nnod{4,2,2,2} % array of number of nodes per layer
		\readlist\Nstr{1,2,3,4} % array of string number of nodes per layer
		\readlist\Cstr{x,h^{(\prev)},h^{(\prev)},y} % array of coefficient symbol per layer
		\def\yshift{0.45} % shift last node for dots
	...
	\begin{tikzpicture}[x=2cm,y=1cm]
		\readlist\Nnod{4,7,2} % array of number of nodes per layer
		\readlist\Nstr{1,2,3} % array of string number of nodes per layer
		\readlist\Cstr{x,h^{(\prev)},y} % array of coefficient symbol per layer
		\def\yshift{0.45} % shift last node for dots
...
\begin{algorithm}
	\caption{Batched stochastic gradient descent with the learning rate $\gamma$ and the Mean Squared Error (\texttt{MSE}) cost function}\label{alg:cap}
	\begin{algorithmic}
