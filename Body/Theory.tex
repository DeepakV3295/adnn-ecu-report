...
The output images will be generated in \texttt{\$BUILDDIR/tmp/deploy/images/imx6qdlsabresd}. To boot the MCIMX6Q-SDB with the build image, flash an SD card with the \textit{rootfs} image and configure the board to boot from the SD card slot. The MCIMX6Q-SDB can be configured to start from the SD-3 slot may be configured in this manner by toggling the dip switches (shown in yellow highlights in Figure \ref{fig:mcimx6q-sdb}) to the D1-OFF D2-ON D3,4,5,6-OFF D7-ON D8-OFF configuration. An interface is then accessible via the USB UART (found beside the dip switches) on the MCIMX6Q-SDB. Use a serial console terminal emulator at 115200 baud after connecting via the micro-B USB plug.

% TODO : Add an overview of the process described and give more brief on the upcoming content
