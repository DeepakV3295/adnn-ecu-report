...
\abstractdummy{
	There is great potential in enabling neural network applications in embedded devices and an important step in that is to allow for these device to performing training on the data points that are generated on board the device. Neural network inference is a popular and well supported function on these platforms however neural network training still has ways to go. In this project we take a closer look at this step and try to compare the performance capabilities of popular machine learning frameworks with straight forward implementation approaches.
}
